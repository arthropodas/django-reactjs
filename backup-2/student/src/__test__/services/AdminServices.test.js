...
describe("adminLogin", () => {
  ...
  it("should call the login endpoint with the provided data", async () => {
    const data = { username: "testuser", password: "testpasP&89d" };
    const response = { token: "exampleToken" };
    mock
      .onPost(`${process.env.REACT_APP_API_ENDPOINT}admin/login`, data)
      .reply(200, response);

    const result = await adminServices.adminLogin(data);
...
describe("admin management apis", () => {
  ...
  it("list questions based on category and page", async () => {
    ...
    mock
      .onGet(
        `admin/question-management/?category_id=${category_id}&page=${page}`
      )
      .reply(200, data);

    const response = await adminServices.adminListQuestions(category_id, page);
    expect(response.status).toBe(200);
    expect(response.data).toEqual(data);
  });
  ...
  test('list all written students successfully', async () => {
    mock.onGet(`admin/exam/1/shortlisted-students?page=1`).reply(200, { students: [] });
  
    const response = await adminServices.adminListAllWrittenStudents(1, 1);
    expect(response.status).toBe(200);
  });
  ...
  test('list feedbacks successfully', async () => {
    mock.onGet(`student/feedbacks?page=1`).reply(200, { feedbacks: [] });
  
    const response = await adminServices.adminListFeedbacks(1);
    expect(response.status).toBe(200);
  });
