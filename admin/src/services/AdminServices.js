...
const adminDashboardCount = (year) => {
  let url = 'admin/dashboard';
  if (year) {
    url += `?yearforShortlistedStudents=${year}`;
  }
  return axiosAdminPrivate.get(url);
};
...
const adminBarGraph = (startDate,endDate) => {
  return axiosAdminPrivate.get(`admin/bar-graph-data?startDate=${startDate}&endDate=${endDate}`);
}
const adminExamAnalytics = () => {
  return axiosAdminPrivate.get("admin/exam-analytics");  
}
const adminExamQuestionnaireAnalytics=(questionnaireId)=>{
  let url = 'admin/exams-questionnaire-analysis';
  if(questionnaireId){
    url = `admin/exams-questionnaire-analysis?questionnaireId=${questionnaireId}`
  }
  return axiosAdminPrivate.get(url);
}
const adminServices = {
